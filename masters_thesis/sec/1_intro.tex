...
Most self-driving vehicles (SDV's) utilize a 3D object detector to recognize and localize objects in 3D space.
...
In this thesis, we present a simple and sensor-agnostic
...
backpropogation through consecutive training examples consumes prohibitive amounts of memory,
training on long sequences can cause over-fitting when backpropagating on every example,
...
\cref{sec:method} describes our architecture (\cref{sec:method-arch}) and
training procedure (\cref{sec:method-training}).
\Cref{sec:experiments} benchmarks our approach against alternatives
(\cref{sec:aug,sec:sota}) and ablates (\cref{sec:ablations}) and
analyzes the effect of (\cref{sec:additional-quantitative-results})
our various contributions. Finally, \cref{sec:imp} describes our implementation details,
and \cref{sec:conclusion} provides a summary and areas for future work.
% \ben{TODO: check and update this section out when format is done}
